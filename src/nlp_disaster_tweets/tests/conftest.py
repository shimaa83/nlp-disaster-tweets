...
@pytest.fixture(scope="session")
def trained_model():
    """Session-scoped fixture to load ONNX predictor once for test suite."""
    settings = get_settings()

    if not settings.onnx_path.exists():
        pytest.skip("ONNX model artifact not found. Run export script first.")

    return DisasterTweetPredictor.load(settings.onnx_path)
